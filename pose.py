...
def custom_objects(robot: cozmo.robot.Robot):
    ...
    if len(cubes) == 1:
      ...
      #  print(robot.pose)
        ...
        print(dis)
        L = dis*sin(atan(cubey/cubex) + (math.pi/2 - cubes[0].pose.rotation.angle_z.radians))
        angle = robot.pose.rotation.angle_z.radians  - (math.pi/2 - cubes[0].pose.rotation.angle_z.radians)
       # angle1=cubes[0].pose.rotation.angle_z.radians-robot.pose.rotation.angle_z.radians
        print('L =  ',L)
        print('angle = ', angle)
       # turn1=math.pi/2-angle1
       # parralleDist=cos(turn1)*dis #if cubey>0 else -cos(turn1)*dis
       # verticalDist=sin(turn1)*dis #if cubex>0 else -sin(turn1)*dis

        #x=cubes[0].pose.position.x
        #y=cubes[0].pose.position.y
        #z=cubes[0].pose.position.z
        
        #a=cubes[0].pose.rotation.angle_z.degrees-robot.pose.rotation.angle_z.degrees
        robot.turn_in_place(radians(angle)).wait_for_completed()#turn to cube
        robot.drive_straight(distance_mm(L),speed_mmps(30)).wait_for_completed()
        
        #robot.turn_in_place(degrees(90)).wait_for_completed()#turn parallel to cube
        #robot.drive_straight(distance_mm(parralleDist),speed_mmps(30)).wait_for_completed()
        #robot.turn_in_place(degrees(cubes[0].pose.rotation.angle_z.degrees-robot.pose.rotation.angle_z.degrees)).wait_for_completed()
